...
class Composer
  ...
  attr_accessor :max_threads, :sgraph, :sfp, :freq
  ...
  def initialize(max_thread, sgraph_file, sfp_file, lowest_pillar, pillar_step)
    ...
    read_access_freq_profile(sfp_file+".pt")
    read_sgraph(sgraph_file)
  end

  # ========================================
  # read the per-thread access frequency file
  # ========================================
  def read_access_freq_profile(filename)

    @freq_profile = SFPCurveGroup.new(@max_threads)
    ...
    File.open(filename,"r").each_line do |l|
      ...
      for i in (0..@max_threads-1)
        ...
        curves[i].sfp << splitted_line[i+1].to_f
      end
    end

    # add curves to CurveGroup
    (1..@max_threads).each do |i|
      @freq_profile.set_curve(curves[i-1], i)
    end

    #dump_sfp(@freq_profile)
  ...
  def read_sgraph(filename)
    ...
    (@gLowestPillar..35).each do |ext|
      ...
      @sgraph << temp
      @gPillarLengths << (1<<(@gLowestPillar + @pillar_step*(ext-@gLowestPillar)))
    end
  ...
  # ==========================================
  # get the thread group's total access frequency
  # ==========================================
  def thread_group_freq(row, thread_group)
  
    ret = 0
    group_list = Composer.bitmap_to_array(thread_group)
    group_list.each do |t|
      ret += @freq_profile[t][row]
    end
    return ret
  ...
  def sfp_partial(thread_group)
    ...
    (0..@curve_length-1).each do |i|
      ...
      freq = thread_group_freq(i, thread_group)

      ret.ft << freq

      #if phase != @gPillarLengths.length-1 and ws > max_ft_at_current_phase
      if ws > max_ft_at_current_phase
        phase = phase + 1
        if phase == @gPillarLengths.length
          max_ft_at_current_phase = @sfp.ft_at(@curve_length-1)+1
        else
          max_ft_at_current_phase = @gPillarLengths[phase]
        end
      ...
      ret.sfp << (1..@max_threads).to_a.inject(0) { |r, k|
        if phase == 0 
        ...
        elsif phase == @gPillarLengths.length
          ...
          #puts "#{porp[phase-1][k]} + (#{ws} - #{@gPillarLengths[phase-1]})/(#{@gPillarLengths[phase]} - #{@gPillarLengths[phase-1]}) * (#{porp[phase][k]} - #{porp[phase-1][k]})" if c < 0
          r += c * @sfp[k][i]
        end
        r
      }
  ...
  def print_partial_sfp(thread_group)

    ret = sfp_partial(thread_group)
    (0..@curve_length-1).each do |i|
      puts "#{ret.ft[i]}\t#{ret.sfp[i]}"
    end
  end
...
c.print_partial_sfp(group_bitmap)
